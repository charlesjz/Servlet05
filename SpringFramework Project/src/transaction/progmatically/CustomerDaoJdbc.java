		...
		transactionTemplate.execute ( 
			new TransactionCallback ( )
			{
				public Object doInTransaction(TransactionStatus ts)
				{
					...
					{
						for ( int k = 0; k < customers.size(); k ++ ) {
							if ( customers.get( k ).getFirstName().trim().equalsIgnoreCase ( "PAUL" ) ) {
								throw new InvalidCustomerNameException ( "No customer can have the first name of \"Paul\"." );
							}
							create ( customers.get( k ) );
						}

//						jdbcTemplate.batchUpdate (
//							crudStatements.getInsertCustomerSql(),
...
//									if ( customer.getFirstName().trim().equalsIgnoreCase ( "PAUL" ) ) {
//										throw new InvalidCustomerNameException ( "No customer can have the first name of \"Paul\"." );
//									}
//									System.out.println ( "Valid name, proceed the customer creation." );
...
//						);
					...
					catch ( Exception e )
					{
						...
						ts.setRollbackOnly();
						if ( e instanceof InvalidCustomerNameException )
						{
							throw (InvalidCustomerNameException)e;
						}
						else
						{
